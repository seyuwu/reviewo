...
describe("PresenceService", () => {
  ...
  it("tracks site visitors separately from entity chat presence", async () => {
    const store = new Map<string, Map<string, number>>();
    const redisService = {
      getClient: async () => ({
        zAdd: async (key: string, entries: Array<{ score: number; value: string }>) => {
    ...
    assert.equal(await service.markSiteVisitor("visitor-1"), 1);
    assert.equal(await service.markSiteVisitor("visitor-2"), 2);
    assert.equal(await service.getSiteVisitorCount(), 2);
  });
});
